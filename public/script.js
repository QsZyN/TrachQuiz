document.addEventListener('DOMContentLoaded', () => {
    ...
    const choiceAccordion = document.getElementById('choice-accordion');
    const choiceAccordionSummary = document.getElementById('choice-accordion-summary');
    ...
    async function init() {
        ...
        if (inputModeToggle) {
            inputModeToggle.addEventListener('click', toggleInputMode);
        }

        if (choiceAccordion && choiceAccordionSummary) {
            choiceAccordion.addEventListener('toggle', () => {
        ...
        await initializeNfcFeatures();
    }

    async function initializeNfcFeatures() {
        try {
            const response = await fetch('./rules/nfc_mapping.json');
            nfcMapping = await response.json();
            nfcIdLookup = buildNfcIdLookup(nfcMapping);
            console.log('NFC mapping loaded:', Object.keys(nfcIdLookup).length, 'tag IDs');
        } catch (error) {
            console.error('Failed to load NFC mapping:', error);
            updateNfcStatus('NFCマッピングの読み込みに失敗しました');
            return;
        }
    ...
    function buildNfcIdLookup(mappingData) {
        ...
        // New format: { choices: { "選択肢": { questionType, nfcIds: [] } } }
        if (mappingData && mappingData.choices && typeof mappingData.choices === 'object') {
            Object.entries(mappingData.choices).forEach(([choice, config]) => {
                if (!config || !Array.isArray(config.nfcIds)) {
                    return;
                }

                const questionType = config.questionType;
                if (questionType !== 'marubatsu' && questionType !== 'bunbetsu') {
                    return;
                }

                config.nfcIds.forEach((id) => {
                    const normalizedId = normalizeTagId(id);
                    if (normalizedId) {
                        lookup[normalizedId] = choice;
                    }
                });
            });
            return lookup;
        ...
        // Backward compatible format: { "NFC_ID": "選択肢" }
        if (mappingData && typeof mappingData === 'object') {
            Object.entries(mappingData).forEach(([id, choice]) => {
                const normalizedId = normalizeTagId(id);
                if (normalizedId && typeof choice === 'string') {
                    lookup[normalizedId] = choice;
                }
            });
        }
    ...
    function updateInputModeUI() {
        if (inputModeToggle) {
            if (!nfcAvailable) {
                ...
                inputModeToggle.textContent = inputMode === 'nfc' ? '入力: NFC' : '入力: クリック';
            }
        }
    }

    function updateChoiceAccordionSummaryText() {
        if (!choiceAccordion || !choiceAccordionSummary) {
            return;
        }
        choiceAccordionSummary.textContent = choiceAccordion.open ? '選択肢を閉じる' : '選択肢を表示';
    }
    ...
    async function loadRulesAndStart(difficulty) {
        try {
            ...
            answerPool = Array.from(new Set(rules.map(item => item.answer))).filter(Boolean);
            bunbetsuAnswerPool = Array.from(
                new Set(
                    rules
                        .filter(item => item.questionType === 'bunbetsu')
    ...
    function loadQuestion() {
        ...
        optionKeys = generateAnswerOptions(item);
        renderCurrentQuestionChoices();

        console.log('Current question:', item.title, 'Answer:', item.answer);
    }

    function generateAnswerOptions(item) {
        const correctAnswer = item.answer;
        const isMarubatsuQuestion = item.questionType === 'marubatsu' || correctAnswer === '〇' || correctAnswer === '×';

        if (isMarubatsuQuestion) {
            return ['〇', '×'];
        }

        const sourcePool = bunbetsuAnswerPool.length > 0 ? bunbetsuAnswerPool : answerPool;
        const candidates = sourcePool.filter(answer => answer !== correctAnswer);
        const shuffled = shuffleArray([...candidates]);
        const options = [correctAnswer, ...shuffled.slice(0, 3)];
        return shuffleArray(options);
    }

    function renderCurrentQuestionChoices() {
        if (!choicePanel || !choiceAccordion || !choiceAccordionSummary) {
            return;
        }

        const canShowChoices = inputMode === 'click';
    ...
    function handleAnswer(selectedKey) {
        ...
        // Keyboard/Click/NFC all compare against the answer string.
        isCorrect = selectedKey === currentItem.answer;
        console.log('Answer detected. Comparing:', selectedKey, 'with answer:', currentItem.answer, 'isCorrect:', isCorrect);
    ...
    function showResult() {
        if (choicePanel && choiceAccordion) {
            choiceAccordion.open = false;
            choiceAccordion.classList.add('hidden');
            choicePanel.innerHTML = '';
        }
